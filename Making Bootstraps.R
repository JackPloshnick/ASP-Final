...
## dem or rep &&  conservative or liberal
...
## Define treats matrix, indicating treatments recieved
...
treat= cbind(type.mat.final, num.mat.final[,1], stage.mat.final[,1:2],party.mat.final[,1], 
...
Xfull.original = model.matrix(~X.original*treat)
...
OOSPredicts = function(x,y, test.indexes = sample(length(y),as.integer(length(y)/10))){
  ...
  # Removing rows with this condition
  ...
  # Fit 12 = SVM-SMO -- requires Java to be installed on computer
...
num.boostraps = 2
# For repeatable but still random results, set seed to pick seednumbers for each bootstrap sample
set.seed(10)

#First number in line below should be any number that is sufficiently large relative to number of bootrstaps
# IMPORTANT, if this is changed, lines 31 and 65 in "Ensemble Weights and Errors" must also be changed.
seednum = sample(10000,num.boostraps) 
results = array(dim = c(1074,9,num.boostraps)) # 1074 observations * 9 models
...
for (i in 1:num.boostraps){
  ...
  # Has a single row of all N/As at end that is removed on next line
